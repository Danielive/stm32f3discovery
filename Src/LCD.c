...
void LCD_WriteIndex(uint16_t index)
{
	...
	GPIOD->ODR = index;	 /* GPIO_Write(GPIOB,index); */
	HAL_Delay(0);
	Clr_nWr;
	Set_nWr;
}
...
void LCD_WriteData(uint16_t data)
{
	Set_Rs;
	HAL_Delay(0);
	GPIOD->ODR = data;	 /* GPIO_Write(GPIOB,data); */
	HAL_Delay(0);
	Clr_nWr;
	Set_nWr;
}
...
uint16_t LCD_ReadData(void)
{
	...
	uint32_t prev_moder = GPIOD->MODER;
	GPIOD->MODER = 0x00000000;
	value = GPIOD->IDR;

	/* PD.00(D0), PD.01(D1), PD.02(D2), PD.03(D3), PD.04(D4), PD.05(D5), PD.06(D6), PD.07(D7), PD.08(D8)
...
void LCD_Initializtion(void)
{
 ...
 // HAL_Delay(100);
//  }
 // GPIO_SetBits(GPIOC, GPIO_PIN_10);  /* Enable back light */
	...
	{
	    LCD_Code = SSD1289;
	    LCD_WriteReg(0x0000,0x0001);    HAL_Delay(50);   /* Enable LCD Oscillator */
	    ...
	    LCD_WriteReg(0x0011,0x6070);    HAL_Delay(50);
	    ...
	    LCD_WriteReg(0x000F,0x0000);    HAL_Delay(50);
